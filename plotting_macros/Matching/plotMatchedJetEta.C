...
void plotMatchedJetEta(void)
{
  ...
  string histName = "matchPartJetPtDetJetEtaPartJetEta";
  string histTitle = "";
  string saveName = "matchedJetEta";
  string xTitle = "#eta^{jet, det}";
  string yTitle = "#eta^{jet, truth}";
  string legendTitle = "";
  ...
  bool setLogY = false;
  bool setLogZ = true;
  double xMinFrame = -1, xMaxFrame = 1, yMinFrame = -1, yMaxFrame = 1, zMinFrame = 1e-5, zMaxFrame = 1;
  ...
  // Histogram stuff
  TH3F* matchedJetPtEtaEta = (TH3F*)matchJetsDir->Get(histName.c_str());
  matchedJetPtEtaEta->Sumw2();
  int firstBinPt = 1, lastBinPt = matchedJetPtEtaEta->GetNbinsX();

  firstBinPt = matchedJetPtEtaEta->GetXaxis()->FindBin(ptMin);
  lastBinPt = matchedJetPtEtaEta->GetXaxis()->FindBin(ptMax);
  matchedJetPtEtaEta->GetXaxis()->SetRangeUser(ptMin, ptMax);
  TH2F* matchedJetEtaEta = (TH2F*)matchedJetPtEtaEta->Project3D("zy");
  matchedJetEtaEta->Rebin2D(rebinNumber, rebinNumber);
  normaliseHistRowByRow(matchedJetEtaEta);
  matchedJetEtaEta->GetZaxis()->SetRangeUser(zMinFrame, zMaxFrame);
  ...
  latexText = "";
  plotOneHist(myCanvas, frame, matchedJetEtaEta, legend, saveName, "", latexText);

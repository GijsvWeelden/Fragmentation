...
void plotMatchedJetPhi(void)
{
  ...
  string histName = "matchPartJetPtDetJetPhiPartJetPhi";
  string histTitle = "";
  string saveName = "matchedJetPhi";
  ...
  bool setLogY = false;
  bool setLogZ = true;
  double xMinFrame = 0, xMaxFrame = 2*TMath::Pi(), yMinFrame = 0, yMaxFrame = 2*TMath::Pi(), zMinFrame = 1e-5, zMaxFrame = 1;
  ...
  // Histogram stuff
  TH3F* matchedJetPtPhiPhi = (TH3F*)matchJetsDir->Get(histName.c_str());
  matchedJetPtPhiPhi->Sumw2();
  int firstBinPt = 1, lastBinPt = matchedJetPtPhiPhi->GetNbinsX();

  firstBinPt = matchedJetPtPhiPhi->GetXaxis()->FindBin(ptMin);
  lastBinPt = matchedJetPtPhiPhi->GetXaxis()->FindBin(ptMax);
  matchedJetPtPhiPhi->GetXaxis()->SetRangeUser(ptMin, ptMax);
  TH2F* matchedJetPhiPhi = (TH2F*)matchedJetPtPhiPhi->Project3D("zy");
  matchedJetPhiPhi->Rebin2D(rebinNumber, rebinNumber);
  normaliseHistRowByRow(matchedJetPhiPhi);
  matchedJetPhiPhi->GetZaxis()->SetRangeUser(zMinFrame, zMaxFrame);
  ...
  latexText = "";
  plotOneHist(myCanvas, frame, matchedJetPhiPhi, legend, saveName, "", latexText);

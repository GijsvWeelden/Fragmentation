...
void plotMatchedJetPt_projected(void)
{
  double time = clock();
  ...
  TH2F* matchedJetPt = (TH2F*)matchJetsDir->Get(histName.c_str());
  matchedJetPt->Sumw2();
  int firstBinPtTruth = 1, lastBinPtTruth = matchedJetPt->GetNbinsY() + 1;
  firstBinPtTruth = matchedJetPt->GetYaxis()->FindBin(ptMin);
  lastBinPtTruth = matchedJetPt->GetYaxis()->FindBin(ptMax);
  TH1F* matchedJetPt_projected = (TH1F*)matchedJetPt->ProjectionX(TString::Format("matchedJetPt_projected_pt%.0f-%.0f", ptMin, ptMax).Data(), firstBinPtTruth, lastBinPtTruth);
  matchedJetPt_projected->Scale(1./matchedJetPt_projected->Integral());
  matchedJetPt_projected->SetLineWidth(3);
  matchedJetPt_projected->SetLineColor(GetColor(0));
  matchedJetPt_projected->SetMarkerStyle(GetMarker(0));
  matchedJetPt_projected->SetMarkerColor(GetColor(0));
  histVector.push_back(matchedJetPt_projected);

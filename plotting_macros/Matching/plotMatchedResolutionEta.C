...
void plotMatchedResolutionEta(void)
{
  double time = clock();
  ...
  string histName = "matchPartJetPtResolutionEta";
  string histTitle = "";
  string saveName = "matchedJetResolutionEta";
  string xTitle = "#eta^{jet, truth} - #eta^{jet, det}";
  string yTitle = "";
  string legendTitle = "";
  ...
  bool setLogY = false;
  bool setLogZ = false;
  double xMinFrame = -0.5, xMaxFrame = 0.55, yMinFrame = 0, yMaxFrame = 1, zMinFrame = 1e-5, zMaxFrame = 1;
  ...
  TH3F* matchPartJetPtResolutionEta = (TH3F*)matchJetsDir->Get(histName.c_str());
  // TH2F* matchPartJetPtResolutionEta = (TH2F*)matchJetsDir->Get(histName.c_str());
  matchPartJetPtResolutionEta->Sumw2();
  int firstBinPt = 1, lastBinPt = matchPartJetPtResolutionEta->GetNbinsX();
  int firstBinEta = 1, lastBinEta = matchPartJetPtResolutionEta->GetNbinsY();

  TH1F* matchedJetResolutionEta = (TH1F*)matchPartJetPtResolutionEta->ProjectionZ(TString::Format("resolutionEta_pt%.0f-%.0f", ptMin, ptMax).Data(), firstBinPt, lastBinPt, firstBinEta, lastBinEta);
  matchedJetResolutionEta->Rebin(rebinNumber);
  matchedJetResolutionEta->Scale(1./matchedJetResolutionEta->Integral());
  matchedJetResolutionEta->SetLineWidth(3);
  matchedJetResolutionEta->SetLineColor(GetColor(0));
  matchedJetResolutionEta->SetMarkerStyle(GetMarker(0));
  matchedJetResolutionEta->SetMarkerColor(GetColor(0));
  histVector.push_back(matchedJetResolutionEta);

...
void plotMatchedResolutionPhi(void)
{
  double time = clock();
  ...
  string histName = "matchPartJetPtResolutionPhi";
  string histTitle = "";
  string saveName = "matchedJetResolutionPhi";
  string xTitle = "#phi^{jet, truth} - #phi^{jet, det}";
  string yTitle = "";
  string legendTitle = "";
  ...
  bool setLogY = false;
  bool setLogZ = false;
  double xMinFrame = -.5, xMaxFrame = .5, yMinFrame = 0, yMaxFrame = 1, zMinFrame = 1e-5, zMaxFrame = 1;
  ...
  TH3F* matchPartJetPtResolutionPhi = (TH3F*)matchJetsDir->Get(histName.c_str());
  matchPartJetPtResolutionPhi->Sumw2();
  int firstBinPt = 1, lastBinPt = matchPartJetPtResolutionPhi->GetNbinsX();
  int firstBinPhi = 1, lastBinPhi = matchPartJetPtResolutionPhi->GetNbinsY();

  TH1F* matchedJetResolutionPhi = (TH1F*)matchPartJetPtResolutionPhi->ProjectionZ(TString::Format("resolutionPhi_pt%.0f-%.0f", ptMin, ptMax).Data(), firstBinPt, lastBinPt, firstBinPhi, lastBinPhi);
  matchedJetResolutionPhi->Rebin(rebinNumber);
  matchedJetResolutionPhi->Scale(1./matchedJetResolutionPhi->Integral());
  matchedJetResolutionPhi->SetLineWidth(3);
  matchedJetResolutionPhi->SetLineColor(GetColor(0));
  matchedJetResolutionPhi->SetMarkerStyle(GetMarker(0));
  matchedJetResolutionPhi->SetMarkerColor(GetColor(0));
  histVector.push_back(matchedJetResolutionPhi);

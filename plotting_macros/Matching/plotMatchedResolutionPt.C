...
void plotMatchedResolutionPt(void)
{
  double time = clock();
  ...
  TH2F* matchPartJetPtResolutionPt = (TH2F*)matchJetsDir->Get(histName.c_str());
  matchPartJetPtResolutionPt->Sumw2();
  int firstBinPt = 1, lastBinPt = matchPartJetPtResolutionPt->GetNbinsX();

  TH1F* matchedJetResolutionPt = (TH1F*)matchPartJetPtResolutionPt->ProjectionY(TString::Format("resolutionPt_pt%.0f-%.0f", ptMin, ptMax).Data(), firstBinPt, lastBinPt);
  matchedJetResolutionPt->Rebin(rebinNumber);
  matchedJetResolutionPt->Scale(1./matchedJetResolutionPt->Integral());
  matchedJetResolutionPt->SetLineWidth(3);
  matchedJetResolutionPt->SetLineColor(GetColor(0));
  matchedJetResolutionPt->SetMarkerStyle(GetMarker(0));
  matchedJetResolutionPt->SetMarkerColor(GetColor(0));
  histVector.push_back(matchedJetResolutionPt);

...
void plot_histograms(TH1F *h_pp, TH1F *h_AA_nr, string obs, double min_pt, double max_pt){
  ...
  if (abs(h_pp->Integral() -  1.0) > 0.01
    ...
    cout << "WARNING: NORMALISATION PROBLEM!" << endl
      ...
      << "AA_norecoil = " << h_AA_nr->Integral() << " (" << h_AA_nr->Integral()-1.0 << ")" << endl;:w
  ...
  h_AA_nr->SetTitle(TString::Format("Jet %s, pt #in [%.0f,%.0f) GeV/c, R = %.1f",
...
void plot_jetprops(void){
  ...
  std::vector<string> obs = {"zg"}; //{"nconst","zg","Rg","nSD","mass","mz2","mr","mr2","rz","r2z"};
  //int ipt = 3;
  //int iobs = 0;
  ...
  for (int iobs=0; iobs<obs.size(); iobs++){
    ...
    for (int ipt=0; ipt<ptBins.size()-1; ipt++){
      ...
      if (AA_rList){
        hAA_recoil = (TH1F*)AA_rList->FindObject(TString::Format("hAA_recoil_h%s_pt%.0f_%.0f",
              obs[iobs].c_str(), ptBins[ipt], ptBins[ipt+1]).Data());
      }

...
{
	int nEvents = 200;
  string outName = "jetclusteringcorrections";
  ...
  for (int iEvent = 0; iEvent < nEvents; iEvent++)
	{
		...
		std::vector<int> matchedJets;
		for (auto& chjet : chJets) {
			if (chjet.pt() < jetptcut) {
				matchedJets.push_back(-1);
				continue;
			}
			...
			double matchDist = jetR/2.;
			for (int i = 0; i < v0Jets.size(); i++) {
				auto& v0Jet = v0Jets[i];
				if (v0Jet.pt() < jetptcut) { continue; }
				// Check if jet is taken
				if (std::find(matchedJets.begin(), matchedJets.end(), i) != matchedJets.end()) {
					continue;
				}
				double dR = v0Jet.delta_R(chjet);
				if (dR < matchDist) {
			...
			int closest = *min_element(distances.begin(), distances.end());
			int closestIndex = possibleMatches[closest];

			auto& v0Jet = v0Jets[closestIndex];
			...
			matchedJets.push_back(closestIndex);
			hChJetMatched->Fill(chjet.pt(), chjet.eta(), chjet.phi());
	...
	hChJetMatched->Write(hChJetMatched->GetName(), TObject::kOverwrite);

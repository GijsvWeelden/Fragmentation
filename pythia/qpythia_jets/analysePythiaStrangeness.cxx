...
void analysePythia(Char_t const *indir = "output/events_40_80_50", Char_t const *foutname = "qPythiaJetSpectra.root", const Int_t maxEvent = 200000) {
  ...
  TH3F *hLeadJetPtEtaZg[nR] = {0};
  TH3F *hLeadJetPtZgDeltaR[nR] = {0};
  TH3F *hLeadJetPtEtaZg1S[nR] = {0};
  TH3F *hLeadJetPtZgDeltaR1S[nR] = {0};
  TH3F *hLeadJetPtEtaZg2S[nR] = {0};
  TH3F *hLeadJetPtZgDeltaR2S[nR] = {0};
  ...
  TH2F *hPtJetPtLambda[nR] = {0};
  TH2F *hPtJetZLambda[nR]= {0};
  TH2F *hPtJetPtKaon[nR] = {0};
  TH2F *hPtJetZKaon[nR] = {0};

  TH2F *hPtJetNLambda[nR] = {0};
  TH2F *hPtJetNKaon[nR] = {0};
  TH2F *hPtJetNLambdaKaon[nR] ={0};
#endif
  ...
  for (Int_t iR = 0; iR < nR; iR++) {
    ...
    hJetPtZgDeltaR[iR] = new TH3F(Form("hJetPtZgDeltaR_R%.0f",10*Rvals[iR]),Form("hJetPtZgDeltaR R=%.1f;p_{T,jet} (GeV);z_{g};#DeltaR",Rvals[iR]),nPtJetBins,minPtJet,maxPtJet,50,0,0.5,10*Rvals[iR],0,Rvals[iR]);
    ...
    hLeadJetPtEtaZg[iR] = new TH3F(Form("hLeadJetPtEtaZg_R%.0f",10*Rvals[iR]),Form("hLeadJetPtEtaZg R=%.1f;p_{T,jet} (GeV);#eta;z_{g}",Rvals[iR]),nPtJetBins,minPtJet,maxPtJet,20,-eta_jet_max,eta_jet_max,50,0,0.5);
    hLeadJetPtZgDeltaR[iR] = new TH3F(Form("hLeadJetPtZgDeltaR_R%.0f",10*Rvals[iR]),Form("hLeadJetPtZgDeltaR R=%.1f;p_{T,jet} (GeV);z_{g};#DeltaR",Rvals[iR]),nPtJetBins,minPtJet,maxPtJet,50,0,0.5,10*Rvals[iR],0,Rvals[iR]);
    hLeadJetPtEtaZg1S[iR] = new TH3F(Form("hLeadJetPtEtaZg1S_R%.0f",10*Rvals[iR]),Form("hLeadJetPtEtaZg1S R=%.1f;p_{T,jet} (GeV);#eta;z_{g}",Rvals[iR]),nPtJetBins,minPtJet,maxPtJet,20,-eta_jet_max,eta_jet_max,50,0,0.5);
    hLeadJetPtZgDeltaR1S[iR] = new TH3F(Form("hLeadJetPtZgDeltaR1S_R%.0f",10*Rvals[iR]),Form("hLeadJetPtZgDeltaR1S R=%.1f;p_{T,jet} (GeV);z_{g};#DeltaR",Rvals[iR]),nPtJetBins,minPtJet,maxPtJet,50,0,0.5,10*Rvals[iR],0,Rvals[iR]);
    hLeadJetPtEtaZg2S[iR] = new TH3F(Form("hLeadJetPtEtaZg2S_R%.0f",10*Rvals[iR]),Form("hLeadJetPtEtaZg2S R=%.1f;p_{T,jet} (GeV);#eta;z_{g}",Rvals[iR]),nPtJetBins,minPtJet,maxPtJet,20,-eta_jet_max,eta_jet_max,50,0,0.5);
    hLeadJetPtZgDeltaR2S[iR] = new TH3F(Form("hLeadJetPtZgDeltaR2S_R%.0f",10*Rvals[iR]),Form("hLeadJetPtZgDeltaR2S R=%.1f;p_{T,jet} (GeV);z_{g};#DeltaR",Rvals[iR]),nPtJetBins,minPtJet,maxPtJet,50,0,0.5,10*Rvals[iR],0,Rvals[iR]);
    ...
    Float_t jetR = Rvals[iR];
    char hname[256];
    char htitle[256];

    sprintf(hname,"hPtJetPtLambda_R%02d",int(10*jetR));
    sprintf(htitle,"(anti-)Lambda p_{T} in jets R=%.1f;p_{T,jet} (GeV/c);p_{T,#Lambda}",jetR);
    hPtJetPtLambda[iR] = new TH2F(hname,htitle,nPtJetBins,minPtJet,maxPtJet,nPtJetBins,minPtJet,maxPtJet);
    hPtJetPtLambda[iR]->Sumw2();
    
    sprintf(hname,"hPtJetZLambda_R%02d",int(10*jetR));
    sprintf(htitle,"(anti-)Lambda in jets R=%.1f;p_{T,jet} (GeV/c);z",jetR);
    hPtJetZLambda[iR] = new TH2F(hname,htitle,nPtJetBins,minPtJet,maxPtJet,nZBins, 0, 1);
    hPtJetZLambda[iR]->Sumw2();
    
    sprintf(hname,"hPtJetPtKaon_R%02d",int(10*jetR));
    sprintf(htitle,"(anti-)Kaon p_{T} in jets R=%.1f;p_{T,jet} (GeV/c);p_{T,K}",jetR);
    hPtJetPtKaon[iR] = new TH2F(hname,htitle,nPtJetBins,minPtJet,maxPtJet,nPtJetBins,minPtJet,maxPtJet);
    hPtJetPtKaon[iR]->Sumw2();
    
    sprintf(hname,"hPtJetZKaon_R%02d",int(10*jetR));
    sprintf(htitle,"(anti-)Kaon in jets R=%.1f;p_{T,jet} (GeV/c);z",jetR);
    hPtJetZKaon[iR] = new TH2F(hname,htitle,nPtJetBins,minPtJet,maxPtJet,nZBins, 0, 1);
    hPtJetZKaon[iR]->Sumw2();

    sprintf(hname,"hPtJetNLambda_R%02d",int(10*jetR));
    sprintf(htitle,"Number of (anti-Lambdas) per jet R=%.1f;p_{T,jet} (GeV/c);N_{#Lambda}",jetR);
    hPtJetNLambda[iR] = new TH2F(hname,htitle,nPtJetBins,minPtJet,maxPtJet,8,0,8);
    hPtJetNLambda[iR]->Sumw2();

    sprintf(hname,"hPtJetNKaon_R%02d",int(10*jetR));
    sprintf(htitle,"Number of kaons per jet R=%.1f;p_{T,jet} (GeV/c);N_{K}",jetR);
    hPtJetNKaon[iR] = new TH2F(hname,htitle,nPtJetBins,minPtJet,maxPtJet,8,0,8);
    hPtJetNKaon[iR]->Sumw2();

    sprintf(hname,"hPtJetNLambdaKaon_R%02d",int(10*jetR));
    sprintf(htitle,"Number of (anti-Lambdas)  plus Kaons per jet R=%.1f;p_{T,jet} (GeV/c);N_{#Lambda+K}",jetR);
    hPtJetNLambdaKaon[iR] = new TH2F(hname,htitle,nPtJetBins,minPtJet,maxPtJet,8,0,8);
    hPtJetNLambdaKaon[iR]->Sumw2();
    ...
    */
	...
	for (UInt_t iJet = 0;  iJet < inclusiveJetsCh.size(); iJet++) {
	  ...
	  if (TMath::Abs(jet.eta()) < eta_jet_max) {
	    ...
	    delta_R = groomed_jet.structure_of<fastjet::contrib::SoftDrop>().delta_R();  // or mu() or delta_R()
	...
	if (lead_pt > 0) {
	  ...
	  hLeadJetPtEtaZg[iR]->Fill(lead_pt,lead_eta,lead_z_g);
	  hLeadJetPtZgDeltaR[iR]->Fill(lead_pt,lead_z_g,lead_delta_R);
	  ...
	  for (Int_t ipart = 0; ipart < plistSel.GetEntriesFast(); ipart++) {
	    ...
	    if (!is_charged(part) && TMath::Abs(part->GetPdgCode()) != kLambda0 && TMath::Abs(part->GetPdgCode()) != kK0 )
	      continue;
      ...
      for (Int_t i_part = 0; i_part < n_part; i_part++) {
	TParticle *part=(TParticle*) stack->Particle(i_part);
       if (TMath::Abs(part->GetPdgCode()) == kLambda0 ||
	    TMath::Abs(part->GetPdgCode()) == kKPlus || TMath::Abs(part->GetPdgCode()) == kK0) {
	    Float_t deta = part->Eta() - lead_eta;
	    if (TMath::Abs(deta) < max_delta_eta) {
	      Float_t dphi = part->Phi() - lead_phi;
	      if (dphi < -TMath::Pi())
		...
		if (Rdist < Rvals[iR]) {
		    if (TMath::Abs(part->GetPdgCode()) == kLambda0) {
		      hPtJetPtLambda[iR]->Fill(lead_pt, part->Pt() );
		      hPtJetZLambda[iR]->Fill(lead_pt, part->Pt()/lead_pt );
		      nLam++;
		    }
		    if (TMath::Abs(part->GetPdgCode()) == kKPlus || TMath::Abs(part->GetPdgCode()) == kK0) {
		      hPtJetPtKaon[iR]->Fill(lead_pt, part->Pt() );
		      hPtJetZKaon[iR]->Fill(lead_pt, part->Pt()/lead_pt );
		      nKa++;
		    }
                 }
             }
          }
	  hPtJetNLambda[iR]->Fill(lead_pt, nLam );
	  hPtJetNKaon[iR]->Fill(lead_pt, nKa );
	  hPtJetNLambdaKaon[iR]->Fill(lead_pt, nLam + nKa );
          if (nLam + nKa == 1) {
            hLeadJetPtEtaZg1S[iR]->Fill(lead_pt,lead_eta,lead_z_g);
	    hLeadJetPtZgDeltaR1S[iR]->Fill(lead_pt,lead_z_g,lead_delta_R);
          }
          else if (nLam + nKa == 2) {
            hLeadJetPtEtaZg2S[iR]->Fill(lead_pt,lead_eta,lead_z_g);
	    hLeadJetPtZgDeltaR2S[iR]->Fill(lead_pt,lead_z_g,lead_delta_R);
          }
       }
#endif

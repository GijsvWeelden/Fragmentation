...
void pythia6JetSpectra(Int_t nEvent = 50, Char_t const *foutname = "qPythiaJetSpectra.root", Float_t pthard_min=10, Float_t pthard_max=100, Float_t qHat = 50., Float_t e_cms = 2760) {
  ...
  header->SetRecombScheme(fastjet::E_scheme);
  header->SetRapRange(-eta_jet_max-0.5, eta_jet_max+0.5);
  header->SetJetEtaMin(-eta_jet_max);
  header->SetJetEtaMax(eta_jet_max);
  ...
  for (Int_t iEvent = 0; iEvent < nEvent; iEvent++) {
	...
	//Int_t n_trk2 = 0;
	//Float_t sum_pt = 0;
	for (Int_t ipart = 0; ipart < aliplist.GetEntriesFast(); ipart++) {
	  AliMCParticle *apart = (AliMCParticle*) aliplist.At(ipart);
	  if (apart == 0) 
	    continue;
	  //cout << "particle " << apart << " phi " << apart->Pt() << endl;
	  Float_t deta = apart->Eta() - lead_eta;
	  if (TMath::Abs(deta) < max_delta_eta) {
	    Float_t dphi = apart->Phi() - lead_phi;
	    if (dphi < -TMath::Pi())
	      dphi += 2*TMath::Pi();
	    if (dphi > TMath::Pi())
	      dphi -= 2*TMath::Pi();

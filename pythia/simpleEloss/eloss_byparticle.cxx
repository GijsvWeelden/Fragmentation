...
double getDeltaE(double Epart) {
  Float_t meanLoss = TMath::Min(0.25*Epart,6.);
  return gRandom->Exp(meanLoss);
}
...
//FASTJET  SETTINGS
	...
	std::vector<fastjet::PseudoJet> fjInputsLoss;

//___________________________________________________ 
//HISTOGRAMS


	TFile* outFile = new TFile(Form("Output_byparticle_relDE025_maxDE6_exp_%s.root",gSystem->Getenv("PBS_JOBID")),"RECREATE");
...
//Begin event loop
	...
	for(int i = 0; i < nEvents; i++)
	{
		...
		if(PythiaJets[0].pt() < minPtJetLoss) continue; // TODO: could remove this condition
		...
		// Fill track histo; apply Eloss
		fjInputsLoss.resize(0);
		for (auto part : fjInputs) {
		  hTrackEtaPt->Fill(part.pseudorapidity(),part.pt());
		  double ptNew = part.pt()-getDeltaE(part.pt());
		  if (ptNew > 0) {
		    double pzNew = sqrt(part.m2()+ptNew*ptNew)*sinh(part.rapidity());
		    double Enew = sqrt(part.m2()+ptNew*ptNew+part.pz()*part.pz());
		    double pxNew = ptNew/part.pt()*part.px();
		    double pyNew = ptNew/part.pt()*part.py();
		    fjInputsLoss.push_back(part);
		    fjInputsLoss.back().reset_momentum(pxNew,pyNew,pzNew,Enew);
		  }
		}
		...
		fastjet::ClusterSequenceArea clustSeq2(fjInputsLoss, *jetDefAKT_Sig, fAreaDef);
		auto ElossJets = clustSeq2.inclusive_jets();

		// Fill track histo
		for (auto part : fjInputsLoss) {
